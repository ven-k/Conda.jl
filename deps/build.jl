...
# Check if JuliaSim is in STDLIB. If yes, it's likely the JuliaSim IDE. Re-write the deps.jl
if "JuliaSim" ∈ readdir(Sys.STDLIB)
    @info "Re-building Conda.jl"
    deps = """
        const MINICONDA_VERSION = "3"
        const ROOTENV = "$(escape_string(abspath(first(DEPOT_PATH), "conda", "3")))"
        const USE_MINIFORGE = true
        const CONDA_EXE = "$(escape_string(abspath(first(DEPOT_PATH), "conda", "3", "bin", "conda")))"
        """
else
